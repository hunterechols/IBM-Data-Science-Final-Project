...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        filtered_df = (spacex_df.loc[spacex_df['class'] == 1]
                        .groupby(['Launch Site', 'class']).size().reset_index(name='class count'))
...
def get_scatter_plot(entered_site, slider_range):
    ...
    if entered_site == 'ALL':
        ...
        title = f'Correlation of Payload Mass to Successful Launches')
        return fig
    else:
        filtered_df = filtered_df.loc[filtered_df['Launch Site'] == entered_site]
        fig = px.scatter(filtered_df, x='Payload Mass (kg)',
                        y = 'class', color='Booster Version Category',
        title = f'Correlation of Payload Mass to Successful Launches for {entered_site}')
        return fig
